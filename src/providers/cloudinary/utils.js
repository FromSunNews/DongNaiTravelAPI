 ...
 * ```
 */
const getResourceInformation = (function() {
  /*
  ...
  */
  ...
  return function(resourceUrl) {
    try {
      ...
      let results = resourceUrl.split(primaryReg)[1].match(fileNameReg)

      return {
        fullFileName: results[1],
        publicId: results[0].split('.')[0],
        resourceType: resourceUrl.match(/(image)|(video)|(raw)|(auto)/g)[0]
      }
    } catch (error) {
